...
bool sequence_definition::check_and_adjust(THD *thd, bool set_reserved_until)
{
  ...
  /*
    If min_value is not set, set it to value_type_min() or 1, depending on
    real_increment
  */
  ...
  /*
    If max_value is not set, set it to value_type_max() or -1, depending on
    real_increment
  */
  ...
  if (is_unsigned && (ulonglong) max_value >= (ulonglong) start &&
      ...
      ((real_increment > 0 && (ulonglong) reserved_until >= (ulonglong) min_value) ||
       (real_increment < 0 && (ulonglong) reserved_until <= (ulonglong) max_value)))
    DBUG_RETURN(FALSE);
