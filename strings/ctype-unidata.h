...
/*TODO: dump this as a single structure: */
extern const uint16 ws_general_ci_page00[256];
extern const uint16 *ws_general_ci_index[256];
extern const uint16 ws_general_mysql500_ci_page00[256];
extern const uint16 *ws_general_mysql500_ci_index[256];
...
{
  const uint16 *page;
  DBUG_ASSERT((wc >> 8) < array_elements(ws_general_ci_index));
  page= ws_general_ci_index[wc >> 8];
...
{
  if ((wc >> 8) < array_elements(ws_general_ci_index))
...
{
  const uint16 *page;
  DBUG_ASSERT((wc >> 8) < array_elements(ws_general_mysql500_ci_index));
  page= ws_general_mysql500_ci_index[wc >> 8];
  return page ? page[wc & 0xFF] : wc;
}
